...
import './style.css';
import { about } from '../utilities/Path';

export const About = () => {
    return (
        <>
            <Element name="home" className="element bg-[#171a1d] pb-2">
                              ...
                              about?.map(({ title, percentage }, id) => {
                                    return <div className='leading-10'>
                                        ...
                                        <div data-name={percentage} className={`p-1 relative bg-[#3b3d3f]   lg:w-[550px] overflow-hidden rounded-full `}>
                                            <div  className={`bg-[#00ffff]  rounded-full absolute left-0 top-0 h-full `} style={{width:`${percentage}%`}}></div>
                                        </div>
                                    </div>
                                })
                            }
